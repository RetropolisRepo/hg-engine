            ...
            if (sp->attack_client != sp->state_client)
            {
                ...
                if (sp->scw[IsClientEnemy(bw, sp->state_client)].mistCount && GetBattlerAbility(sp, sp->attack_client) != ABILITY_INFILTRATOR)
                {
                    ...
                    flag = 1;
                      ...
                      && (sp->battlemon[sp->state_client].type1 == TYPE_GRASS || sp->battlemon[sp->state_client].type2 == TYPE_GRASS || sp->battlemon[sp->state_client].type3 == TYPE_GRASS)) // and target has grass type
                {
